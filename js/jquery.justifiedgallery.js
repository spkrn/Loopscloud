...
(function($){
   ...
   $.fn.justifiedGallery = function(options){
			...
			$(cont).find("a").each(function(index, entry){
				...
				$(img).load(function() {
					...
					var usedSizeRangeRegExp = new RegExp("(" + settings.sizeRangeSuffixes.lt100 + "|" 
														 ...
														 + settings.sizeRangeSuffixes.lt1000 + "|" 
		...
		function startProcess(cont, images, settings){
			...
			$(cont).find(".jg-loading").fadeOut(0, function(){
		...
		function getSuffix(nw, nh, settings){
			...
			if(n <= 100){
				...
				return settings.sizeRangeSuffixes.lt1000; //medium (longest side:1000)
			}else{
...
// C'EST ICI QU'IL FAUT MODIFIER PREPEND POUR APPEND POUR CHANGER L ORDRE D APPARITION DES LIGNES..
			...
			$(cont).prepend(buildContRow(row, images, extraW, settings));
			//---------------------------
			...
			if(settings.captions){
				$(cont).find(".jg-image").mouseenter(function(sender){
					...
					$(sender.currentTarget).find(".jg-image-label").fadeTo(500, 1.0);
				});
